...
<head>
    ...
    <style>
        ...
        .form-header{background:linear-gradient(135deg,#1a3c5e 0%,#0d6efd 100%);border-radius:8px 8px 0 0;padding:28px 32px 20px;color:#fff;}
        .form-header h1{font-size:1.35rem;font-weight:700;margin-bottom:6px;}
        ...
        .guide-bar{background:#fffbeb;border-bottom:1px solid #fde68a;padding:10px 32px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;}
        .form-body{background:#fff;padding:28px 32px;border-radius:0 0 8px 8px;box-shadow:0 4px 24px rgba(0,0,0,.08);}
        .q-block{border-left:3px solid #1a3c5e;padding:16px 20px;margin-bottom:20px;background:#f8fafc;border-radius:0 8px 8px 0;transition:border-color .2s;}
        ...
        .stat-preview{background:linear-gradient(135deg,#f0f9ff 0%,#e0f2fe 100%);border:1px solid #bae6fd;border-radius:10px;padding:16px 20px;margin:20px 0;}
        ...
        .btn-submit{background:#1a3c5e;border:none;color:#fff;padding:12px 32px;border-radius:6px;font-size:.95rem;font-weight:600;width:100%;cursor:pointer;transition:background .2s;}
        ...
        .success-card{background:#fff;border-radius:12px;box-shadow:0 4px 24px rgba(0,0,0,.08);padding:48px 32px;text-align:center;}
    </style>
</head>
<body>
...
<div class="form-header">
    <div class="d-flex align-items-center mb-3" style="gap:12px;">
        <img src="https://media.licdn.com/dms/image/v2/D560BAQH-4NKYZuKb5A/company-logo_200_200/B56ZYgesv1HoAI-/0/1744301641123?e=1775692800&v=beta&t=T35fKEIIQZ0XooMkYKRdQPwY91QRxjBheWWmw49cLFU"
             alt="SCM" style="width:44px;height:44px;border-radius:8px;object-fit:cover;background:rgba(255,255,255,.15);">
        <div>
            <div style="font-size:.72rem;opacity:.7;letter-spacing:1px;text-transform:uppercase;">PT Sulawesi Cahaya Mineral</div>
    ...
    <div style="margin-top:10px;font-size:.78rem;opacity:.7;" id="form-notice">* Wajib diisi</div>
</div>
...
<div class="guide-bar">
    ...
    <span style="font-size:.8rem;color:#92400e;" id="guide-text">Sebelum mengisi, baca panduan pengisian terlebih dahulu.</span>
    <a href="{{ route('form.guide') }}" id="btn-guide"
       style="margin-left:auto;background:#d97706;color:#fff;border-radius:20px;padding:4px 14px;font-size:.78rem;font-weight:600;text-decoration:none;white-space:nowrap;">
        <i class="bi bi-file-earmark-pdf me-1"></i><span id="guide-btn-text">Download Panduan PDF</span>
    </a>
</div>
    ...
    <form action="{{ route('form.store') }}" method="POST" novalidate>
        ...
        <div class="q-block">
            ...
            <input type="number" name="total_manpower"
                ...
                value="{{ old('total_manpower') }}" min="1" id="input-mp"
        ...
        <div class="q-block">
            ...
            <input type="number" name="total_pob"
                ...
                value="{{ old('total_pob') }}" min="1" id="input-pob"
                placeholder="0" required>
...
<script>
// ================================================================
// KAMUS TERJEMAHAN — semua teks yang tampil di halaman ada di sini
// ================================================================
const LANG = {
    id: {
        // Header
        ...
        // Panduan bar
        guideText    : 'Sebelum mengisi, baca panduan pengisian terlebih dahulu.',
        guideBtnText : 'Download Panduan PDF',
        // Error
        errLabel     : 'Harap periksa isian berikut:',
        // Pertanyaan
        ...
        successSub   : 'Data telah tercatat di sistem PT Sulawesi Cahaya Mineral',
        btnBackText  : 'Kirim Laporan Lain',
    },
    en: {
        formTitle    : 'Daily Reporting MP & POB',
        ...
        formNotice   : '* Required',
        guideText    : 'Please read the filling guide before submitting.',
        guideBtnText : 'Download Guide PDF',
        errLabel     : 'Please check the following fields:',
        ...
        q4desc:'Total POB (Personal on Board) is the number of employees currently in the IUP PT SCM area up to MHR only. Employees working at representative offices in Kendari and Morowali do not need to be included.',
        ...
        successSub   : 'Data has been recorded in the PT Sulawesi Cahaya Mineral system',
        btnBackText  : 'Submit Another Report',
    },
    zh: {
        formTitle    : 'MP & POB 每日报告',
        formSubtitle : '请确保您提交的数据正确无误，因为这些数据将用于每周二的会议管理。',
        formNotice   : '* 必填',
        guideText    : '填写前请先阅读填写指南。',
        guideBtnText : '下载指南 PDF',
        errLabel     : '请检查以下填写内容：',
        ...
        successTitle : '报告提交成功！',
        successSub   : '数据已记录在 PT Sulawesi Cahaya Mineral 系统中',
        btnBackText  : '提交另一份报告',
    },
};

// ================================================================
// FUNGSI GANTI BAHASA — update semua elemen sekaligus
// ================================================================
function setLang(lang) {
    const t = LANG[lang];
    if (!t) return;

    // Aktifkan tombol bahasa yang dipilih
    ...
    set('form-subtitle', t.formSubtitle);
    set('form-notice',   t.formNotice);

    // Panduan bar
    set('guide-text',     t.guideText);
    set('guide-btn-text', t.guideBtnText);

    // Error label
    set('err-label', t.errLabel);

    // Pertanyaan label & nomor
    for (let i = 1; i <= 6; i++) {
        set(`q${i}-num`,   t[`q${i}num`]);
        set(`q${i}-label`, t[`q${i}label`]);
    }

    // Deskripsi Q3 & Q4
    set('q3-desc', t.q3desc);
    set('q4-desc', t.q4desc);

    // Placeholder select Q1
    const selPh = document.getElementById('q1-ph');
    if (selPh) selPh.textContent = t.q1ph;

    // Placeholder input
    setP('input-informed', t.q5ph);
    setP('input-wa',       t.q6ph);

    // Stat preview labels
    set('prev-mp-lbl',    t.prevMpLbl);
    set('prev-pob-lbl',   t.prevPobLbl);
    set('prev-ratio-lbl', t.prevRatioLbl);

    // Tombol submit
    set('btn-submit-text', t.btnSubmit);

    // Footer
    setH('footer-note', t.footerNote);

    // Halaman sukses
    set('success-title',  t.successTitle);
    set('success-sub',    t.successSub);
    set('btn-back-text',  t.btnBackText);
...
// ================================================================
// PREVIEW STATISTIK (MP, POB, Rasio)
// ================================================================
...
// ================================================================
// INIT — load bahasa terakhir yang dipilih
// ================================================================
